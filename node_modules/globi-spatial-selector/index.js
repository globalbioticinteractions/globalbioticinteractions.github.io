...
SpatialSelector.prototype.update = function (search) {
    this.emit('spatialselector:update', searchContext);
};
...
SpatialSelector.prototype.appendMap = function (target, params) {
    ...
    var updateSearchContext = function () {
        var param = null;
        if (drawnItems.getLayers().length > 0) {
            param = bboxUtil.fromBounds(drawnItems.getLayers()[0].getBounds())['bbox'];
        }
    ...
    me.on('spatialselector:update', function (searchContext) {
        me.searchContext = searchContext;
        deleteSelectedArea();
        addSelectedArea(me.searchContext.searchParameters);
        fitBounds();
    });
