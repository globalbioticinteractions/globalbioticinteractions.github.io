    ...
    showList: function(data, downloadlinks) {
        ...
        if (data.length > 0) {
            ...
            var linkRefs = downloadlinks.map(function(link) {
                var downloadText = 'download ' + link.resultType + ' with up to 4096 records';
                return '<a href="' + link.url + '" class="link" title="' + downloadText + '">' + downloadText + '</a>';
            }).join(' ');
            tableHead.innerHTML = [
                '<tr><th class="source-cell">' + linkRefs + '</th>',
                ('<th></th>'),
                '<th class="target-cell"><a href="/data">more download options</a></th></tr>',
                '<tr>',
